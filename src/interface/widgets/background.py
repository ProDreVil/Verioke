...
warnings.filterwarnings("ignore", message=r"CTkLabel Warning: Given image is not CTkImage.*", category=UserWarning)
...
class BackgroundVideo(ctk.CTkFrame):
    ...
    def select_video(self):
        ...
        if not folder.exists():
            return
        videos = [path for path in folder.iterdir() if path.suffix.lower() in [".mp4", ".avi", ".mov", ".mkv"]]
        if not videos:
            return
    ...
    def update_frame(self):
        ...
        if fps <= 0:
            fps = 30
        target_delay = 1 / (fps * self.playback_speed)
        elapsed = time.perf_counter() - start
        remaining = target_delay - elapsed
        self.after(max(1, int(remaining * 1000)), self.update_frame)
